...
public class CommandeSourceTest {
	...
	{
		fichier = new File(this.getClass().getClassLoader().getResource("testCommandeFile").getFile());
	}
	...
	public void testCreateCommande() {
		...
		Assert.assertEquals(NotationCardinale.N, commandes.get(0).getPosition().getOrientation());
		String operations = commandes.get(0).getOperations()
				.stream ()
				.map (i -> i.toString ())
		...
		Assert.assertEquals(NotationCardinale.E, commandes.get(1).getPosition().getOrientation());
		operations = commandes.get(1).getOperations()
				.stream ()
				.map (i -> i.toString ())
				.collect (Collectors.joining (""));
	...
	public void testInitSurface(){
		Surface surface = CommandeSource.initSurface(fichier);
